...
ถ้าอยู่ใน Cloud Shell และยังไม่แน่ใจว่าเคย clone repo แล้วหรือยัง ให้ใช้ block นี้ได้เลย:

```bash
if [ -d "lego-firebase/.git" ]; then
  cd lego-firebase
...
> พร้อมใช้: block นี้กันได้ทั้ง 2 error ที่เจอบ่อย คือ `$REPO_URL` ว่าง และ `fatal: destination path 'lego-firebase' already exists and is not an empty directory`

ถ้าหน้าจอขึ้นว่า `destination path 'lego-firebase' already exists` แปลว่า clone ไว้แล้ว ให้เข้า folder เดิมแทน:
